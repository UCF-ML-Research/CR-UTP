...
class Trainer:
    ...
    def _train_step(
        ...
        loss, batch_log, prompt_trigger_dic_train, prompt_trigger_dic_val = model(
            batch, clean_prompt, target_label, prompt_trigger_dic_train, prompt_trigger_dic_val, prompts, attack_type
        )
        loss.backward()
        self.train_op()
            ...
            for step, batch in enumerate(train_dataloader):
                batch_log, prompt_trigger_dic_train, _ =\
                    self._train_step(
                        ...
                        prompt_trigger_dic_train, prompt_trigger_dic_val, prompts=config['prompts'], attack_type=config['attack_type']
                    )
                if report_to_wandb:
                    wandb.log(batch_log)
                total_steps += 1
